    ...
    function ($scope, $stateParams, $rootScope, $state, $ionicHistory, $ionicPlatform, $http, $ionicModal,
        ...
        $scope.showSignUpPage = function (index) {
            ...
            if ((trackUrl != undefined) && (trackUrl != null) && (trackUrl != '')) {
                trackUrl = trackUrl.replace('{planID}', option.id);
                trackUrl = trackUrl.replace('{type}', plan.connectionType);
                trackUrl = trackUrl.replace('{speed}', Math.round(option.speedDnld) + 'Mbps' + '/' +
                    Math.round(option.speedUpld) + 'Mbps');
                trackUrl = trackUrl.replace('{data}', (option.data > 0) ? option.data + 'GB' : 'Unlim');
                trackUrl = trackUrl.replace('{price}', '$' + option.price);
            ...
            .success(function (plansList) {
                ...
                if ($scope.plansList[$scope.planIndex].options == null) {
                    ...
                    requestStr = addMainParamsToRequestStr(requestStr, plansFilterProvider.getPlansFilter(),
                        plansFilterProvider.getDefaultPlansFilter(), config);
